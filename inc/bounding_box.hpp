...
struct bb_context_packet
{
	...
	float d_input[INPUT_ELEMENTS];
    float d_bbox[BBOX_ELEMENTS];
    float d_cov[COV_ELEMENTS];
	...
	bb_context_packet() {
		...
		raw_frame.create(FRAME_X, FRAME_Y, CV_8UC3);
...
class bounding_box {
	public:
		...
		void runInference(bb_context_packet& bb_context);
